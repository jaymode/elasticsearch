...
public class RepositoriesIT extends AbstractSnapshotIntegTestCase {
    ...
    public void testRepositoryVerification() {
        ...
        ElasticsearchAssertions.assertThrows(client.admin().cluster().preparePutRepository("test-repo-1")
                        .setType("mock").setSettings(settings),
                RepositoryVerificationException.class);

        logger.info("-->  creating read-only repository that cannot read any files - should fail");
        ElasticsearchAssertions.assertThrows(client.admin().cluster().preparePutRepository("test-repo-2")
        ...
        ElasticsearchAssertions.assertThrows(client.admin().cluster().prepareVerifyRepository("test-repo-1"),
            RepositoryVerificationException.class);
        ...
        logger.info("-->  verifying repository");
        ElasticsearchAssertions.assertThrows(client.admin().cluster().prepareVerifyRepository("test-repo-2"),
            RepositoryVerificationException.class);
