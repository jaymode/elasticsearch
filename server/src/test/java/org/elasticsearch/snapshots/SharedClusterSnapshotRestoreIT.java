...
public class SharedClusterSnapshotRestoreIT extends AbstractSnapshotIntegTestCase {
    ...
    public void testDeleteSnapshotWithCorruptedGlobalState() throws Exception {
        ...
        ElasticsearchAssertions.assertThrows(client().admin().cluster().prepareSnapshotStatus("test-repo").addSnapshots("test-snap"),
    ...
    public void testReadonlyRepository() throws Exception {
        ...
        ElasticsearchAssertions.assertThrows(client.admin().cluster().prepareDeleteSnapshot("readonly-repo", "test-snap"),
            RepositoryException.class,
            "cannot delete snapshot from a readonly repository");

        logger.info("--> try making another snapshot");
        ElasticsearchAssertions.assertThrows(client.admin().cluster().prepareCreateSnapshot("readonly-repo", "test-snap-2")
    ...
    public void testSnapshotMoreThanOnce() throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < builders.length; i++) {
            builders[i] = client().prepareIndex("test").setId(Integer.toString(i)).setSource("foo", "bar" + i);
        }
    ...
    public void testChangeSettingsOnRestore() throws Exception {
        ...
        ElasticsearchAssertions.assertThrows(client.admin().cluster()
